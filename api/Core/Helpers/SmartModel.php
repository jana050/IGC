...
class SmartModel 
{
   ...
   /**
    ...
    {
        $this->_limit_sql = $limit;
        return $this;
    }
     public function Recursive(string $recursive)
    {
    ...
    public function getDbData(array $data_in = [])
    {
        ...
        $sql_str = sprintf("SELECT %s FROM %s ", $select, $this->_from_sql);
         if (strlen($this->_recursive) > 4) {
            $sql_str  = $this->_recursive . " " . $sql_str;
        }
        //echo $sql_str;
        ...
        // limit 
        $limit = !empty($this->_limit_sql) ? implode(",", $this->_limit_sql) : "";
        if (strlen($limit) > 0 ){
            $sql_str .= " LIMIT " .$limit;
        }
