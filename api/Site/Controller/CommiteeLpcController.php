...
class CommiteeLpcController extends BaseController
{
    ...
    public function insert()
    {
        ...
        // validations
        $this->_commitee_lpc_helper->validate(CommiteeLpcHelper::validations, $columns, $this->post);

        // additional fields
        $columns[] = "created_time";
        $columns[] = "sd_mt_userdb_id";
        $columns[] = "lpc_approver_id";
        $columns[] = "lpc_approver_remarks";
        $columns[] = "status";
        $this->post["status"] = 10;
        $columns[] = "fund_available";
        

        // insert
        $id = $this->_commitee_lpc_helper->insert($columns, $this->post);
        $sub_data = isset($this->post["sub_data"]) ? $this->post["sub_data"] : [];
        if (!empty($sub_data)) {
            $this->_commitee_lpc_sub_helper->insert_update($id, $sub_data);
        }
         // process the file
        $file_path = $this->_commitee_lpc_helper->getFullFile($id);
        ...
        $update_data = ["doc_loc"=>$stored_file_path];
           $this->_commitee_lpc_helper->update($update_columns,$update_data,$id);   
        } 
        // ✅ ADD HISTORY FOR USER SUBMIT 👇
        $this->addCommiteeLpcHistory(  $id,  "EMPLOYEE",  "SUBMITTED",  "LPC request submitted");

        // log
        $this->addLog("RAISED AN LPC COMPLAINT", "", SmartAuthHelper::getLoggedInUserName());
        $this->response($id);
    }
    ...
    public function update()
    {
        ...
        // insert and get id
        $id = $this->_commitee_lpc_helper->update($columns, $this->post, $id);
        if (!empty($sub_data)) {
            $this->_commitee_lpc_sub_helper->insert_update($id, $sub_data);
        }
         // process the file
        ...
        $this->addCommiteeLpcHistory( $id, "EMPLOYEEE", "UPDATED", "LPC request updated");
        // add log
        $this->addLog("UPDATED AN LPC COMPLAINT", "", SmartAuthHelper::getLoggedInUserName());
        $this->response($id);
    }
    ...
    public function getAll()
    {
        ...
        switch ($mode) {
            ...
            //     break;
            case 'lpc_approver':
                $sql = "t1.lpc_approver_id = :lpc_approver_id AND status IN (" . implode(",", $status) . ")";
                $data_in = ["lpc_approver_id" => SmartAuthHelper::getLoggedInId()];
                break;
            case 'lpc_chairman':
                $sql = "t1.status IN (" . implode(",", $status) . ")";
                break;
        ...
        $data = $this->_commitee_lpc_helper->getAllData($sql, $data_in);
         foreach ($data as $obj) {
            $obj->sub_data = $this->_commitee_lpc_sub_helper->getAllWithParentId($obj->ID);
        }
        $this->response($data);
    }
    ...
    public function getOne()
    {
        ...
        // Get the main record
        $data = $this->_commitee_lpc_helper->getOneData($id);
        ...
        $statusTracker = $this->_commitee_lpc_helper->getStatusTracker($data->status, $data->created_by);

        // Add only status_list to the main data
        $data->status_list = $statusTracker['status_list'];
         if (isset($data->ID)) {
            $data->sub_data = $this->_commitee_lpc_sub_helper->getAllWithParentId($data->ID);
        }
        // Send response
        $this->response($data);
    }
    ...
    public function deleteOne()
    {
        ...
        // insert and get id
        $this->_commitee_lpc_helper->deleteOneId($id);
        // add log
        $this->addLog("DELETED AN LPC COMPLAINT", "", SmartAuthHelper::getLoggedInUserName());
        //
        $out = new \stdClass();
    ...
    public function updateApprovalHos()
    {
        ...
        $id = $this->_commitee_lpc_helper->update($columns, $dt, $id);
         // ✅ ADD HISTORY HERE 👇
        // $this->addCommiteeLpcHistory( $id, "hos", ($status == 15 ? "APPROVED" : "REJECTED"),  $remarks);
        $this->addCommiteeLpcHistory( $id, "HOS", ($status == 15 ? "APPROVED" : "REWORK"), $remarks);


        $logMsg = $status == 15 ? "APPROVED LPC CARD   BY HOS" : "SENT IIBCC CARD FOR REWORK BY HOS";
    ...
    public function updateApprovalHod()
    {
        ...
        $id = $this->_commitee_lpc_helper->update($columns, $dt, $id);
        //
        // $this->addCommiteeLpcHistory( $id, "hod", ($status == 20 ? "APPROVED" : "REJECTED"),  $remarks);
        $this->addCommiteeLpcHistory($id,"HOD",($status == 20 ? "APPROVED" : "REWORK"), $remarks);

        $logMsg = $status == 20 ? "APPROVED LPC CARD   BY HOD" : "SENT LPC CARD FOR REWORK BY HOD";
        $this->addLog($logMsg, $remarks, SmartAuthHelper::getLoggedInUserName());

        $this->response($id);
    }
        public function updateApprovalLpcApprover()
    {
        $id = isset($this->post["id"]) ? intval($this->post["id"]) : 0;
        if ($id < 1) {
            \CustomErrorHandler::triggerInvalid("Invalid ID");
        ...
        $columns = ["status", "lpc_approver_id", "lpc_approver_remarks", "lpc_approver_time"];
        $dt = [
            "status" => $status,
            "lpc_approver_remarks" => $remarks
        ];

        $id = $this->_commitee_lpc_helper->update($columns, $dt, $id);
        // $this->addCommiteeLpcHistory( $id, "lpc_approver", ($status == 25 ? "APPROVED" : "REJECTED"),  $remarks);
        $this->addCommiteeLpcHistory(  $id, "LPC APPROVER",($status == 25 ? "APPROVED" : "REWORK"),$remarks);

        $logMsg = $status == 25 ? "APPROVED LPC BY LPC APPROVER" : "SENT LPC CARD BY LPC APPROVER TO REWORK";
        $this->addLog($logMsg, $remarks, SmartAuthHelper::getLoggedInUserName());

        $this->response($id);
    }
         public function updateApprovalLpcChairman()
    {
        $id = isset($this->post["id"]) ? intval($this->post["id"]) : 0;
        if ($id < 1) {
            \CustomErrorHandler::triggerInvalid("Invalid ID");
        ...
        $columns = ["status", "lpc_chairman_id", "lpc_chairman_remarks", "lpc_chairman_time"];
        $dt = [
            "status" => $status,
            "lpc_chairman_remarks" => $remarks
        ];

        $id = $this->_commitee_lpc_helper->update($columns, $dt, $id);
        // $this->addCommiteeLpcHistory( $id, "lpc_chairman", ($status == 30 ? "APPROVED" : "REJECTED"),  $remarks);
        $this->addCommiteeLpcHistory( $id, "LPC CHAIRMAN", ($status == 30 ? "APPROVED" : "REWORK"), $remarks);

        


        $logMsg = $status == 30 ? "APPROVED LPC BY LPC CHAIRMAN" : "SENT LPC CARD BY LPC CHAIRMAN TO REWORK";
        $this->addLog($logMsg, $remarks, SmartAuthHelper::getLoggedInUserName());
    ...
    public function getPdf()
    {
       ...
       $data = $this->_commitee_lpc_helper->getOneData($id);
       $items = $this->_commitee_lpc_sub_helper->getAllWithParentId($id);      
        // var_dump($data);
        // exit();
        $this->_commitee_lpc_helper->generateCommiteeLpcPdf($id, $data);
          // Correct call (main + sub)
        $html = CommiteeLpcPdf::getHtml((array)$data, $items);
        $path = "commiteelpc" . DS . $id . DS . "commiteelpc.pdf";
    ...
    public function updateUser()
    {
        ...
        $this->post["iibcc_no"] = $this->_commitee_lpc_helper->generateCommiteeIibccNumber();
        // do validations
        $this->_commitee_lpc_helper->validate(CommiteeLpcHelper::validations, $columns, $this->post);
        // add columns
        $columns[] = "last_modified_by";
        ...
        $this->addCommiteeLpcHistory($id,"EMPLOYEEE", "UPDATED","LPC request updated");
        // add log
        $this->addLog("UPDATED AN COMMITEE LPC COMPLAINT", "", SmartAuthHelper::getLoggedInUserName());
        $this->response($id);
    }
    //
        ...
        $data = $this->_commitee_lpc_helper->getOneData($id);
        // 
        $pdf_path =  $this->_commitee_lpc_helper->getFullFile($id) .".pdf";
       // echo $pdf_path;
        $this->responseFileBase64(SmartFileHelper::getDataPath()  . $pdf_path);
    }
       public function addCommiteeLpcHistory($commiteeId, $role, $action, $remarks = "")
       {
       $data = [
        "commitee_name" => "LPC",
     ...
     public function getCommiteeLpcHistory()
    {
    ...
    $data = $this->_commitee_history_helper->getByCommiteeId("LPC", $commitee_id);
