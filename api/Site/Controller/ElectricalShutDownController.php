...
class ElectricalShutDownController extends BaseController
{
    ...
    public function insert()
    {
        ...
        $this->db->_db->Begin();
        // insert and get id
        $id = $this->_elec_shutdown_helper->insert($columns, $this->post);
        // upload the document
        $file_path = $this->_elec_shutdown_helper->getFullFile($id);
        if (isset($_FILES["uploaded_file"])) {
            ...
            $this->_elec_shutdown_helper->update($update_columns, $update_data, $id);
        }
    ...
    public function update()
    {
        ...
        // do validations
        $this->_elec_shutdown_helper->validate(ElectricalShutDownHelper::validations, $valid_columns, $this->post);
        $columns = ["status", "admin_remarks"];
        // add columns
        $columns[] = "last_modified_by";
        $columns[] = "last_modified_time";
        // insert and get id
        $id = $this->_elec_shutdown_helper->update($columns, $this->post, $id);
        // add log
    ...
    {
        $pdf_path =  $this->_elec_shutdown_helper->getFullFile($id) . ".pdf";
     ...
     public function getOnePdf()
    {
      ...
      //  $data =  $this->_elec_shutdown_helper->getOneData($id);
        // 
        $pdf_path =  $this->_elec_shutdown_helper->getFullFile($id) . ".pdf";
        // echo $pdf_path;
    ...
    public function getAll()
    {
        ...
        $data = $this->_elec_shutdown_helper->getAllData($sql, $data_in);
        $out = [];
        foreach ($data as $obj) {
    ...
    */
    ...
    $statusTracker = $this->_elec_shutdown_helper->getStatusTracker($data->status, $data->created_by);
    ...
    public function deleteOne()
    {
        ...
        // insert and get id
        $this->_elec_shutdown_helper->deleteOneId($id);
        // add log
    ...
    public function updateApprovalHos()
    {
        ...
        $id = $this->_elec_shutdown_helper->update($columns, $dt, $id);

        $logMsg = $status == 25 ? "APPROVED ELECTRICAL SHUTDOWN  BY HOS" : "REJECTED ELECTRICAL SHUTDOWN  BY HOS";
    ...
    public function updateApprovalHod()
    {
        ...
        $status = ($status === "approve") ? 35 : (($status === "reject") ? 34 : 0);
        if ($status < 1) {
            \CustomErrorHandler::triggerInvalid("Invalid Action");
        }
       ...
       $columns = ["status","hod_id","hod_remarks","hod_time"];
        ...
        $logMsg = $status == 35 ? "APPROVED ELECTRICAL SHUTDOWN  BY HOD" : "REJECTED ELECTRICAL SHUTDOWN  BY HOD";
        $this->addLog($logMsg, $remarks, SmartAuthHelper::getLoggedInUserName());
    ...
    {
        $sql = "t1.status=25";
        $data = $this->_elec_shutdown_helper->getAllData($sql);
        $this->response($data);
    }
