...
use Site\Helpers\MomIibccHelper;


class MomIibccController extends BaseController
{
    private MomIibccHelper $_mom_iibcc_helper;

    function __construct($params)
    {
        parent::__construct($params);
        // 
        $this->_mom_iibcc_helper = new MomIibccHelper($this->db);
    }
   ...
   ---
    ...
    public function insertMom()
    {
        ...
        // do validations
        $this->_mom_iibcc_helper->validate(MomIibccHelper::validations, $columns, $this->post);
        // add other columns
       $columns[] = "mom_type";
       $this->post["mom_type"] = "IIBCC";
        $columns[] = "created_by";
        $columns[] = "created_time";
        // insert and get id
        $id = $this->_mom_iibcc_helper->insertMom($columns, $this->post);
        //
        $file_path = $this->_mom_iibcc_helper->getFullFile($id);
        ...
        $this->_mom_iibcc_helper->updateMom($update_columns, $update_data, $id);
        // add log
    ...
    public function updateMom()
    {
        ...
        $this->_mom_iibcc_helper->validate(MomIibccHelper::validations, $columns, $this->post);
        // insert and get id
        $id = $this->_mom_iibcc_helper->updateMom($columns, $this->post, $id);
        // add log
    ...
    {
        $data = $this->_mom_iibcc_helper->getAllDataMom();
        $this->response($data);
    }
    /**
     * 
    ...
    public function getOneMom()
    {
        ...
        $data = $this->_mom_iibcc_helper->getOneDataMom($id);
        $this->response($data);
    }
    ...
    public function deleteOneMom()
    {
        ...
        // insert and get id
        $this->_mom_iibcc_helper->deleteOneIdMom($id);
        // add log
    ...
    public function getAllByMomType()
    {
        ...
        $data = $this->_mom_iibcc_helper->getByMomType($type);
        $this->response($data);
    }
       ...
       public function getMomPdf()
{
    ...
    $data = $this->_mom_iibcc_helper->getOneByMomType($mom_type);

    if (empty($data) || empty($data->ID)) {
        \CustomErrorHandler::triggerInvalid("Document not found");
    }

    $pdf_path = $this->_mom_iibcc_helper->getFullFile($data->ID) . ".pdf";
