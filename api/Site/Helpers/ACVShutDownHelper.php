 ...
 * @author kms
 */
class ACVShutDownHelper extends BaseHelper
{
    ...
    const validations = [
        "from_date" => [
            [
                ...
                "msg" => "Please Enter ACV Shutdown Start Date"
            ]
        ],
        "to_date" => [
            [
                "type" => SmartConst::VALID_REQUIRED,
                "msg" => "Please Enter ACV Shutdown End Date"
            ]
        ],
        "from_time" => [
            [
                "type" => SmartConst::VALID_REQUIRED,
                "msg" => "Please Enter ACV Shutdown Start Time"
            ]
        ],
        "to_time" => [
            [
                "type" => SmartConst::VALID_REQUIRED,
                "msg" => "Please Enter ACV Shutdown End Time"
            ]
        ],
        "uploaded_file" => [
            [
    ...
    {
        return $this->insertDb(self::schema, Table::ACV_SHUTDOWN, $columns, $data);
    }
    /**
     * 
     */
    ...
    {
        return $this->updateDb(self::schema, Table::ACV_SHUTDOWN, $columns, $data, $id);
    }
    /**
     * 
     */
     ...
     * 
     */
    public function getAcvShutDownWithUserID($id)
    {
        $from = Table::ACV_SHUTDOWN;
        $select = ["*"];
    ...
    {
        $select = ["COUNT(description) AS acv_shutdown, MONTH(last_modified_time) AS month"];
        $from = Table::ACV_SHUTDOWN;
        ...
        $acv_shutdown_count = array_fill(0, 12, 0);
        foreach ($count as $acv_shut) {
            $month = intval($acv_shut->month);
            $acvShutCount = intval($acv_shut->acv_shutdown);
            $acv_shutdown_count[$month - 1] = $acvShutCount;
        }
        $acv_shut_count_by_year = array_values($acv_shutdown_count);
        return $acv_shut_count_by_year;
    }
