...
class SiteHelper extends BaseHelper
{
    ...
    const FILE_DPR = "dpr_file";
    const FILE_IIBCC = "iibcc_file";
    const FILE_LPC = "lpc_file";
    ...
    public function getDprFile($id)
    {
        return self::FILE_FOLDER . DS . $id . DS . self::FILE_DPR;
    }
    public function getIibccFile($id)
    {
        return self::FILE_FOLDER . DS . $id . DS . self::FILE_IIBCC;
    }
    public function getLpcFile($id)
    {
        return self::FILE_FOLDER . DS . $id . DS . self::FILE_LPC;
    }
    ...
    public function getFileBase64BySetting($setting_name)
    {
    $fileName = $this->getOneValue($setting_name); // eg: org_one_file.pdf

    if (empty($fileName)) {
        return "";
    }

    // settings/1/org_one_file.pdf
    $relativePath = self::FILE_FOLDER . DS . 1 . DS . pathinfo($fileName, PATHINFO_FILENAME);

    $fullPath = SmartFileHelper::getDataPath() . $relativePath . ".pdf";
