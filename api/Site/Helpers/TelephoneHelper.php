 ...
 * @author kms
 */
class TelephoneHelper extends BaseHelper
{

    const schema = [
        "title" => SmartConst::SCHEMA_VARCHAR,
        "description" => SmartConst::SCHEMA_TEXT,
        "telephone_number" => SmartConst::SCHEMA_TEXT,
    ...
    {
        return $this->insertDb(self::schema, Table::TELEPHONE, $columns, $data);
    }
    /**
     * 
     */
    ...
    {
        return $this->updateDb(self::schema, Table::TELEPHONE, $columns, $data, $id);
    }
    /**
     * 
     */
     ...
     * 
     */
    public function getTeleWithUserID($id)
    {
        $from = Table::TELEPHONE;
        $select = ["*"];
    ...
    public function getCountByYear($year){
        $select = ["COUNT(title) AS tele, MONTH(last_modified_time) AS month"];
        $from = Table::TELEPHONE;
        ...
        $tele_count = array_fill(0, 12, 0);
        foreach ($count as $tele) {
            $month = intval($tele->month);
            $teleCount = intval($tele->tele);
            $tele_count[$month - 1] = $teleCount;
        }
        $tele_count_by_year = array_values($tele_count);
        return $tele_count_by_year;
    }
    ...
    //TelephoneComplaintReport
    public function TelephoneComplaintReport($start_date, $end_date)
    {
    $select = [
        "t1.title",
        "t1.description",
